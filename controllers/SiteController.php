...
class SiteController extends Controller
{
    ...
    public function actionRegister()
    {
        ...
        if ($model->load($request = Yii::$app->request->post()) && $model->register(Yii::$app->params['adminEmail'])) {
            ...
            return $this->renderContent('<div><p/><p/><p/><p class="alert alert-success"> Registro finalizado: ' . ArrayHelper::isAssociative($request) . '<br/>1:<br/>' . print_r($request) . '<br/>2:<br/>' . print_r($response) . '</p></div>');
            //return $this->refresh();
        }
