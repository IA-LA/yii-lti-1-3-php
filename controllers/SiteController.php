    ...
    public function actionQuery()
    {
        ...
        if ($model->load($request = Yii::$app->request->post()) && $model->query(Yii::$app->params['adminEmail'])) {
            ...
            if ($response->isOk && $response->data['result'] === 'ok') {
                $content = '<div><p/><p/><p/><p class="alert alert-success"> Registro: ' . $response->data['result'] . '<br/>ID: ' . $response->data['data']['launch_parameters']['iss'] . '<br/>URL: ' . $response->data['data']['launch_url'] . '</p></div>';
            } else {
                $content = '<div><p/><p/><p/>';
