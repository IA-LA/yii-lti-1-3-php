    ...
    <?php if (Yii::$app->session->hasFlash('uploadFormSubmitted')): ?>
        ...
        <div>
            ...
            <?php
                    ...
                    $retval=null;
                    umask(0000);
                    exec(escapeshellcmd('unzip -X uploads/' . $file . ' -d uploads/publicacion/' . $namedir), $output, $retval);
                    ...
                    echo "<p><pre> 6.a. Unzip PassThru " . passthru('unzip -X uploads/' . $file . ' -d uploads/publicacion/' . $namedir . ' 2>&1') . "<br/>";
                    //print_r($output);
                    //echo "</pre></p>";
                    $output = shell_exec(escapeshellcmd('unzip -X uploads/' . $file . ' -d uploads/publicacion/' . $namedir));
